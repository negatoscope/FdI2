...
shinyServer(function(input, output, session){
	...
	output$distName <- renderUI({
		if(input$disttype=="Discreta"){
			radioButtons("dist","Distribución:",selected="bin",
		...
		isolate({
			if(length(input$dist)){
				lab <- switch(input$dist,
					bern="Probability:", bin="Tamaño:", dunif="Discrete sequence minimum:", geom="Probability:", hgeom="M:", nbin="Number of successes:",	poi="Mean and Variance:", # discrete
					beta="Alpha:", cauchy="Location:", chisq="Grados de libertad:", exp="Rate:", F="Numerador grados de libertad:", gam="Shape:", lap="Location:",
					logi="Location:", lognorm="Mean(log):", norm="Media:", pareto="Location:",	t="Grados de libertad:", unif="Minimum:", weib="Shape:"
					)
		...
		isolate({
			if(length(input$dist)){
				lab <- switch(input$dist,
					...
					beta="Beta:", cauchy="Scale:", F="Denominador grados de libertad:", gam="Rate:", lap="Scale:", # continuous
					logi="Scale:", lognorm="Standard deviation(log)", norm="Desviación estándar:", pareto="Shape:", unif="Maximum:", weib="Scale:"
					)
				ini <- switch(input$dist,
